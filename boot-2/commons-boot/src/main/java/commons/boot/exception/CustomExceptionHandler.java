...
public class CustomExceptionHandler {
    ...
    @ResponseBody
    public Response shiroException(UnauthorizedException e){
        String errorMsg = e.getMessage();
        String msg = "";
        if (errorMsg.contains("not have role")){
            msg = "用户角色不匹配";
        }
        else if (errorMsg.contains("not have permission")){
            msg = "用户操作权限不足，请联系管理员";
        }
        log.error(msg+":"+errorMsg);
        return RespUtil.error(403,msg);
    }
